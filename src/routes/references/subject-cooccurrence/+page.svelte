<script lang="ts">
	...
	// Type definitions for subject co-occurrence network
	interface SubjectNetworkNode extends GlobalNetworkNode {
		type: 'subject';
	}

	interface SubjectNetworkEdge extends GlobalNetworkEdge {
		type: 'subject-subject';
	}

	interface SubjectNetworkMeta {
		generatedAt: string;
		totalNodes: number;
		totalEdges: number;
	...
	interface SubjectNetworkData {
		nodes: SubjectNetworkNode[];
		edges: SubjectNetworkEdge[];
		meta: SubjectNetworkMeta;
	}
	...
	let { data: pageData } = $props<{
		data: {
			networkData: SubjectNetworkData | null;
			error: string | null;
		};
	}>();
	...
	let selectedNode = $state<SubjectNetworkNode | null>(null);
	...
	const filteredNodeIds = $derived(new Set(filteredNodes.map((n: SubjectNetworkNode) => n.id)));

	// Filter edges - both endpoints must be visible
	const filteredEdges = $derived.by(() => {
		if (!networkData) return [];
		return networkData.edges.filter(
			(e: SubjectNetworkEdge) =>
	...
	const egoNetworkData = $derived.by(() => {
		...
		const egoNodes = networkData.nodes.filter((n: SubjectNetworkNode) => neighborIds.has(n.id));

		const egoNodeIds = new Set(egoNodes.map((n: SubjectNetworkNode) => n.id));
		const validEgoEdges = egoEdges.filter(
			(e: SubjectNetworkEdge) => egoNodeIds.has(e.source) && egoNodeIds.has(e.target)
		);
	...
	const nodeSizeOptions: { value: NodeSizeBy; label: string }[] = [
		{ value: 'count', label: 'subject_cooccurrence.size_by_references' },
		{ value: 'degree', label: 'subject_cooccurrence.size_by_connected' },
		{ value: 'strength', label: 'subject_cooccurrence.size_by_cooccurrences' }
	];

	// Sync URL -> state on load
	$effect(() => {
		if (networkData && urlEntityId && !selectedNode) {
			const node = networkData.nodes.find((n: SubjectNetworkNode) => n.id === urlEntityId);
			if (node) {
				selectedNode = node;
	...
	// Event handlers
	function handleNodeClick(node: SubjectNetworkNode | null) {
		selectedNode = node;
		focusMode = false;
	...
	function handleSearchSelect(node: SubjectNetworkNode | null) {
		selectedNode = node;
		focusMode = false;
...
<svelte:head>
	<title>{t('nav.subject_cooccurrence')} | {t('app.title')}</title>
</svelte:head>
		...
		<Card.Root class="p-6">
			<div class="py-12 text-center">
				...
				<p class="mt-4 text-sm text-muted-foreground">
					{t('subject_cooccurrence.run_script_hint')}
				</p>
			</div>
		</Card.Root>
	{:else if networkData}
					...
					<div class="flex flex-col gap-3 sm:flex-row sm:items-center">
						<div class="w-full sm:w-72 lg:w-80">
							<NetworkEntitySearch
								...
								onSelect={(node) => handleSearchSelect(node as SubjectNetworkNode | null)}
							/>
						</div>
						{#if selectedNode}
							<Button
		...
		<Card.Root class="relative overflow-hidden">
			<div class="h-150">
				<NetworkGraph
					...
					{focusMode}
					onNodeClick={(node) => handleNodeClick(node as SubjectNetworkNode | null)}
				/>
				...
				>
					...
					<div class="flex items-center gap-2">
						<div class="h-3 w-3 rounded-full" style:background-color="#22c55e"></div>
						<Tag class="h-3.5 w-3.5" style="color: #22c55e" />
						<span class="text-muted-foreground">{t('subject_cooccurrence.subjects')}</span>
					</div>
		...
		<Card.Root>
			<Card.Content class="py-4">
				<p class="text-sm text-muted-foreground">
					...
					{t('subject_cooccurrence.instructions')}
				</p>
			</Card.Content>
		</Card.Root>
	{/if}
